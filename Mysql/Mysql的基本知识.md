...
- **行锁（Record Lock）**：锁定单个索引记录，阻止其他事务修改或删除该记录。
- **间隙锁（Gap Lock）**：锁定索引记录之间的间隙（不包括记录本身），阻止其他事务插入新记录。
- **临键锁（Next-Key Lock）**：行锁 + 间隙锁的组合，锁定记录本身及前一个间隙，是 InnoDB 的默认锁算法。

### **二、间隙锁退回行锁的核心条件**

间隙锁的释放或退化取决于事务的隔离级别、查询条件及索引使用情况。以下是几种典型场景：

#### **1. 隔离级别为 RC（Read Committed）**

- **原理**：RC 隔离级别下，InnoDB 不使用间隙锁（仅在**外键约束检查**和**重复键检查**时除外），因此间隙锁会自动退化为行锁。
...
存在，则仅锁定该记录；若不存在，则不产生任何锁。

#### **2. 查询条件使用唯一索引且精确匹配（=）**

- **原理**：当查询通过唯一索引（如主键）精确匹配单条记录时，InnoDB 会将临键锁退化为行锁，仅锁定匹配的记录。
...
之间），也不会被锁定，其他事务仍可在该间隙插入数据。

#### **3. 查询条件使用主键或唯一索引的范围查询，但条件为`>`且包含记录**

- **原理**：当查询条件为`id > 10`且存在`id=10`的记录时，InnoDB 会锁定`id=10`的记录及之后的间隙，但不包括`id=10`之前的间隙。
...
不存在，则间隙锁覆盖整个范围（类似临键锁）。

#### **4. 查询条件使用二级索引（非唯一）且精确匹配（=），但同时通过主键过滤**

- **原理**：当查询通过二级索引精确匹配，并通过主键进一步过滤时，间隙锁会被限制在主键记录上。
...
- 示例
  ...
  -- 假设name字段有非唯一索引
  BEGIN;
  SELECT * FROM users WHERE name = 'Alice' AND id = 20 FOR UPDATE;
  COMMIT;
  ```
...
1. **间隙锁的退化与查询优化器有关**：
    - 若查询未使用索引（如全表扫描），间隙锁可能不会退化，甚至导致锁表。
    - 需通过`EXPLAIN`分析查询是否走索引。
2. **外键约束与重复键检查**：
    - 在 RC 隔离级别下，外键约束检查和重复键检查仍会使用间隙锁，以防止幻读。
3. **间隙锁的释放时机**：
    - 间隙锁在事务提交或回滚后释放，与行锁的释放时机一致。

### **四、总结**

间隙锁退回行锁的核心场景是：**在 RC 隔离级别下**或**通过唯一索引精确匹配单条记录时**。此时，InnoDB 会避免锁定间隙，仅锁定具体的记录，从而减少锁的粒度，提高并发性能。理解这些场景有助于优化事务隔离级别和查询语句，减少锁等待和死锁风险。